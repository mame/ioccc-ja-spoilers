...
hint_url: "https://www.ioccc.org/2018/yang/index.html"
...
それぞれのコードの左上のコードが有効になり、右上は一行コメントで無効化、左下と右下は`/* */`または`#if 0 ... #endif`で無効化する、というのが基本的なテクニック（ただし一部のコード断片はその原則を外れている）。
欠けた部分をシフトされている判定はさらに巧妙で、コメントで無効化される縁のあたりでコードが1文字だけ割り込んだり消えたりすることで実現している模様。

同作者のコード遊びが遺憾なく発揮されており、[[[2011/akari]]]({{< relref "/2011/akari/" >}})以来の大作と言える。
