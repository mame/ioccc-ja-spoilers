...
vik2.hintで作者自身が言及している通り、ネタは[[[1988/applin]]]({{< relref "/1988/applin/" >}})と同じだが、よりストイックな実装となっている。
`#define`でマクロを定義するが、右辺値を常に省略する方針で実装されている（`#define X`と定義したら、`X`の展開は空だが`#ifdef X`は真になる）。
